(function SidebarConfig(init = true) {
	// STICKY container
	let appItems = document.querySelector(".main-navBar-entryPoints");
	...
	if (!appItems || !playlistItems || !rootList || !sidebar) {
		setTimeout(() => SidebarConfig(init), 300);
		return;
	}
...
// We will probably need to refactor and remove these duplicate functions but I'm too dumb to do it
(function SidebarXConfig(init = true) {
	// STICKY container
	let appItems = document.querySelector(".main-yourLibraryX-navItems");
	let libraryItems = document.querySelector(".main-yourLibraryX-library");
	const sidebar = document.querySelector(".Root__nav-bar");

	if (!appItems || !libraryItems) {
		setTimeout(() => SidebarXConfig(init), 300);
		return;
	}
	...
	function writeStorage() {
		const array = ordered.map(a => [a[0].dataset.id, a[1]]);
		localStorage.setItem("spicetify-sidebar-config:ylx", JSON.stringify(array));
	}
...
`;
	...
	const sidebarConfigItem = new Spicetify.Menu.Item("Sidebar X config", false, self => {
		self.isEnabled = !self.isEnabled;
		if (self.isEnabled) {
			injectInteraction();
		} else {
