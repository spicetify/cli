const CreditFooter = react.memo(({ provider, copyright }) => {
  const credit = [
    Spicetify.Locale.get("web-player.lyrics.providedBy", provider),
  ];
  if (copyright) {
...
const IdlingIndicator = ({ isActive, progress, delay }) => {
  return react.createElement(
    ...
    react.createElement("div", {
      className: `lyrics-idling-indicator__circle ${
    ...
    return react.createElement(
      "span",
      {
        className:
          ...
          (isWordActive ? " lyrics-lyricsContainer-Karaoke-WordActive" : ""),
        style: {
...
const SyncedLyricsPage = react.memo(
  ({ lyrics = [], provider, copyright, isKara }) => {
    ...
    const activeLines = useMemo(() => {
      const startIndex = Math.max(
        activeLineIndex - 1 - CONFIG.visual["lines-before"],
        0
      );
      // 3 lines = 1 padding top + 1 padding bottom + 1 active
      const linesCount =
        CONFIG.visual["lines-before"] + CONFIG.visual["lines-after"] + 3;
      return lyricWithEmptyLines.slice(startIndex, startIndex + linesCount);
    }, [activeLineIndex, lyricWithEmptyLines]);

    let offset = lyricContainerEle.current
      ? lyricContainerEle.current.clientHeight / 2
      : 0;
    if (activeLineEle.current) {
      offset += -(
        activeLineEle.current.offsetTop +
        activeLineEle.current.clientHeight / 2
      );
    }
    ...
    return react.createElement(
      ...
      react.createElement(
        ...
        activeLines.map(({ text, lineNumber, startTime }, i) => {
          ...
          let activeElementIndex = Math.min(
            activeLineIndex,
            CONFIG.visual["lines-before"] + 1
          );
          let ref;
                ...
                await Spicetify.Platform.ClipboardAPI.copy(rawLyrics)
                  ...
                  .catch(() =>
                    Spicetify.showNotification(
                      "Failed to copy lyrics to clipboard"
                    )
                  );
              },
            },
...
class SearchBar extends react.Component {
  ...
  componentDidMount() {
    ...
    this.mainViewOffsetTop =
      document.querySelector(".Root__main-view").offsetTop;
    this.toggleCallback = () => {
    ...
    Spicetify.Mousetrap(this.container).bind(
      "mod+shift+f",
      this.toggleCallback
    );
  ...
  componentWillUnmount() {
    ...
    Spicetify.Mousetrap(this.container).unbind(
      "mod+shift+f",
      this.toggleCallback
    );
    Spicetify.Mousetrap(this.container).unbind(
...
const SyncedExpandedLyricsPage = react.memo(
  ({ lyrics, provider, copyright, isKara }) => {
    const [position, setPosition] = useState(0);
...
const GeniusPage = react.memo(
  ({
    ...
    return react.createElement(
      ...
      react.createElement("p", {
        className:
          "lyrics-lyricsContainer-LyricsUnsyncedPadding main-type-ballad",
      }),
      react.createElement(
        "div",
        { className: shouldSplit ? "split" : "" },
        mainContainer
      ),
      react.createElement(CreditFooter, {
        provider,
