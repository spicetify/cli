...
const corsProxyTemplate = () => {
  ...
  return react.createElement("input", {
    ...
    onChange: (event) => {
      ...
      if (value === "" || !value)
        return localStorage.removeItem("spicetify:corsProxyTemplate");
...
function openConfig() {
  const configContainer = react.createElement(
    ...
    react.createElement("span", {
      dangerouslySetInnerHTML: {
        __html:
          "Spotify will reload its webview after applying. Leave empty to restore default: <code>https://cors-proxy.spicetify.app/{url}</code>",
      },
    })
  );
