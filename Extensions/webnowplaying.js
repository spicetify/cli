...
class WNPReduxWebSocket {
  ...
  updateSpicetifyInfo(data) {
    ...
    this.spicetifyInfo.Artist = meta.album_title // Podcast
...
function OnMessageLegacy(self, message) {
  ...
  try {
    ...
    switch (type) {
      ...
      case 'SETVOLUME': Spicetify.Player.setVolume(parseInt(data) / 100); break;
      case 'REPEAT': {
...
function SendUpdateLegacy(self) {
  ...
  Object.keys(self.spicetifyInfo).forEach((key) => {
    try {
      ...
      if (key === 'State')
        value = value === 'PLAYING' ? 1 : value === 'PAUSED' ? 2 : 0
      else if (key === 'Repeat')
        value = value === 'ALL' ? 2 : value === 'ONE' ? 1 : 0
      else if (key === 'Shuffle')
        value = value ? 1 : 0
...
function OnMessageRev1(self, message) {
  ...
  try {
    switch (type) {
      ...
      case 'SET_VOLUME': Spicetify.Player.setVolume(parseInt(data) / 100); break
      case 'TOGGLE_REPEAT': {
        Spicetify.Player.toggleRepeat()
        self.spicetifyInfo.Repeat = self.spicetifyInfo.Repeat === 'NONE' ? 'ALL' : self.spicetifyInfo.Repeat === 'ALL' ? 'ONE' : 'NONE'
        break
