...
if (!navigator.serviceWorker) {
    // Worker code
    // When Spotify client is minimized, requestAnimationFrame does not call our tick function
...
function PopupLyrics() {
    ...
    const userConfigs = {
        ...
        fontSize: Number(LocalStorage.get("popup-lyrics:font-size")) || 46,
        blurSize: Number(LocalStorage.get("popup-lyrics:blur-size")) || 10,
        fontFamily: LocalStorage.get("popup-lyrics:font-family") || "spotify-circular",
        width: Number(LocalStorage.get("popup-lyrics:width")) || 600,
        height: Number(LocalStorage.get("popup-lyrics:height")) || 600,
        hPadding: Number(LocalStorage.get("popup-lyrics:h-padding")) || 50, // Added horizontal padding
        vPadding: Number(LocalStorage.get("popup-lyrics:v-padding")) || 50, // Added vertical padding
        delay: Number(LocalStorage.get("popup-lyrics:delay")) || 0,
        services: {
            netease: {
    ...
    lyricVideo.width = userConfigs.width;
    lyricVideo.height = userConfigs.height;
    ...
    function drawBackground(ctx, image) {
        if (userConfigs.showCover) {
            ...
            ctx.drawImage(image, -blurSize * 2, -blurSize * 2, width + 4 * blurSize, height + 4 * blurSize);
            ctx.restore();
    ...
    function drawText(ctx, text, color = "white") {
        ...
        ctx.fillStyle = color;
        ctx.font = `${fontSize}px ${userConfigs.fontFamily}, sans-serif`;
        drawParagraph(ctx, text, {
            vCenter: true,
            hCenter: true,
            left: userConfigs.hPadding, // Apply horizontal padding
            right: userConfigs.hPadding, // Apply horizontal padding
            lineHeight: fontSize,
        });
        ctx.restore();
    }
    ...
    function renderLyrics(ctx, lyrics, currentTime) {
        ...
        const focusLineMargin = userConfigs.vPadding; // Changed to use vertical padding
        const otherLineFontSize = focusLineFontSize * 1;
        const otherLineHeight = otherLineFontSize * 1.2;
        const otherLineMargin = userConfigs.vPadding; // Changed to use vertical padding
        ...
        offscreenCtx.font = `${fFontSize}px ${fontFamily}`;
    ...
    async function tick(options) {
        ...
        if (error) {
            ...
            drawText(lyricCtx, audio.duration ? "Loading" : "Waiting");
        }
...
#popup-lyrics-width-input,
#popup-lyrics-height-input,
#popup-lyrics-h-padding-input,
#popup-lyrics-v-padding-input {
    background-color: rgba(var(--spice-rgb-shadow), .7);
    color: var(--spice-text);
}
`;
            ...
            // New options for Width and Height
            const width = createOptionsInput("Width (px)", String(userConfigs.width), (state) => {
                const widthValue = Math.max(100, Number(state)); // Minimum width of 100px
                userConfigs.width = widthValue;
                LocalStorage.set("popup-lyrics:width", widthValue);
                lyricVideo.width = widthValue;
                lyricCanvas.width = widthValue;
                coverCanvas.width = widthValue;
                offscreenCtx = null;
            });
            const height = createOptionsInput("Height (px)", String(userConfigs.height), (state) => {
                const heightValue = Math.max(100, Number(state)); // Minimum height of 100px
                userConfigs.height = heightValue;
                LocalStorage.set("popup-lyrics:height", heightValue);
                lyricVideo.height = heightValue;
                lyricCanvas.height = heightValue;
                coverCanvas.height = heightValue;
                offscreenCtx = null;
            });

            // New options for Horizontal and Vertical Padding
            const hPadding = createOptionsInput("Horizontal Padding (px)", String(userConfigs.hPadding), (state) => {
                const hPaddingValue = Math.max(0, Number(state)); // Minimum padding of 0px
                userConfigs.hPadding = hPaddingValue;
                LocalStorage.set("popup-lyrics:h-padding", hPaddingValue);
                offscreenCtx = null;
            });
            const vPadding = createOptionsInput("Vertical Padding (px)", String(userConfigs.vPadding), (state) => {
                const vPaddingValue = Math.max(0, Number(state)); // Minimum padding of 0px
                userConfigs.vPadding = vPaddingValue;
                LocalStorage.set("popup-lyrics:v-padding", vPaddingValue);
                offscreenCtx = null;
            });

            const delay = createOptionsInput("Delay (ms)", String(userConfigs.delay), (state) => {
                const delayValue = Number(state);
                userConfigs.delay = delayValue;
                LocalStorage.set("popup-lyrics:delay", delayValue);
            });
            ...
            function stackServiceElements() {
                serviceContainer.innerHTML = ""; // Clear existing elements
                userConfigs.servicesOrder.forEach((name, index) => {
                    const el = userConfigs.services[name].element;

                    const [up, down] = el.querySelectorAll("button.small");
                    if (index === 0) {
                        up.disabled = true;
                    } else {
                        up.disabled = false;
                    }
                    if (index === userConfigs.servicesOrder.length - 1) {
                        down.disabled = true;
                    } else {
                        down.disabled = false;
                    }
            ...
            function posCallback(el, dir) {
                ...
                if (newPos < 0 || newPos >= userConfigs.servicesOrder.length) return; // Prevent out-of-bounds
            ...
            configContainer.append(
                ...
                width,
                height,
                hPadding, // Add horizontal padding option to UI
                vPadding, // Add vertical padding option to UI
                delay,
                serviceHeader,
                serviceContainer
            );
