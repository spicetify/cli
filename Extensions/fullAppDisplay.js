...
<div id="fad-foreground">
    ...
    if (!CONFIG.showAlbumArt) {
      const coverWrapper = container.querySelector("#fad-art");
      coverWrapper.style.display = "none";
      coverWrapper.style.minWidth = 0;
      coverWrapper.style.width = 0;
    }
  }
  ...
  function animateCanvas(prevImg, nextImg) {
    ...
    const blur = CONFIG.blurBackgroundArt ? 30 : 0;
    const brightness = CONFIG.darkenBackgroundArt ? 0.6 : 1;
  ...
  function activate() {
    ...
    if (CONFIG.enableControl) {
      updateControl({
        data: {
          is_paused: !Spicetify.Player.isPlaying(),
        },
      });
  ...
  function setDefaults() {
    ...
    config["blurBackgroundArt"] = true;
    config["darkenBackgroundArt"] = true;
    return config;
  }
...
button.switch.disabled {
    color: rgba(var(--spice-rgb-text), .3);
      ...
      configContainer.append(
        ...
        newMenuItem("Show album art", "showAlbumArt"),
        newMenuItem("Blur background art", "blurBackgroundArt"),
        newMenuItem("Darken background art", "darkenBackgroundArt"),
