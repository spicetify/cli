...
func exposeAPIs_main(input string) string {
	...
	// Context Menu hook
	utils.Replace(&input, `children:([\w_$]+),onClose:[\w_$]+,getInitialFocusElement:[\w_$]+,onFocusVerticalItem:[\w_$]+.+?\}\)=>\{`, `${0}${1}=[...Spicetify.ContextMenuV2.renderItems(Spicetify.React.useContext(Spicetify.ContextMenuV2._context)),...${1}];`)

	utils.Replace(&input, `\(0,([\w_$]+)\.jsx\)\([\w_$]+\.[\w_$]+,\{value:"contextmenu"[^\}]+\}\)\}\)`, `${1}.jsx((Spicetify.ContextMenuV2._context||(Spicetify.ContextMenuV2._context=i.createContext({}))).Provider,{value:e.props,children:${0}})`)
