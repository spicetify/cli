...
pub fn run(ctx: &AppContext, block: bool) -> Result<()> {
    ...
    if backups_exist {
        logging::info("Spotify updates are already blocked (backups exist)");
        return Ok(());
    }
    ...
    let mut dll_data = fs::read(&dll).context("Failed to read Spotify.dll")?;
    let mut exe_data = fs::read(&exe).context("Failed to read Spotify.exe")?;
    let mut elf_data = fs::read(&elf).context("Failed to read chrome_elf.dll")?;

    strip_pe_signature(&mut dll_data).context("Failed to remove signature from Spotify.dll")?;
    strip_pe_signature(&mut exe_data).context("Failed to remove signature from Spotify.exe")?;
    strip_pe_signature(&mut elf_data).context("Failed to remove signature from chrome_elf.dll")?;

    patch_sig_check(&mut dll_data).context("Failed to patch signature check in Spotify.dll")?;

    if !patch_update_url(&mut dll_data) {
        bail!("Failed to find update URL pattern in Spotify.dll");
    }

    fs::copy(&dll, &dll_bak).context("Failed to create Spotify.dll.backup")?;
    fs::copy(&exe, &exe_bak).context("Failed to create Spotify.exe.backup")?;
    fs::copy(&elf, &elf_bak).context("Failed to create chrome_elf.dll.backup")?;

    fs::write(&dll, &dll_data)?;
    fs::write(&exe, &exe_data)?;
    fs::write(&elf, &elf_data)?;

    logging::info("Updates blocked successfully");
    Ok(())
}
...
fn unblock(
    ...
    if backups_exist {
        ...
        logging::info("Updates unlocked (restored from backups)");
    } else {
        logging::info("Backups not found. Please reinstall Spotify to unlock updates");
    }
    Ok(())
}
...
fn patch_sig_check(data: &mut [u8]) -> Result<()> {
    ...
    let str_offset = util::find_bytes(data, needle)
        .ok_or_else(|| anyhow!("Signature check string not found in Spotify.dll"))?
        as u32;
    ...
    if patch_offset == 0 {
        bail!("Could not find reference to signature check function");
    }
